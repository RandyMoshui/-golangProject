...
func myWeb(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Info("internal IP fetch failed, detail:" + err.Error())
	}
	defer conn.Close()
	...
	httpflv_addr := configure.Config.GetString("httpflv_addr")
	t, _ := template.ParseFiles("./static/index.html")
	data := map[string]string{
		"flvaddress": res+httpflv_addr,
		"innerip": res,
	}
	t.Execute(w, data)
}

func startHTML(stream *rtmp.RtmpStream){
	htmlAddr := configure.Config.GetString("html_addr")
	apiAddr := configure.Config.GetString("api_addr")
	rtmpAddr := configure.Config.GetString("rtmp_addr")

	go func() {
		fmt.Println("aaaa",htmlAddr)
		...
		err := http.ListenAndServe(htmlAddr, nil)
		if err!= nil {
			fmt.Println("html服务启动出错！", err)
		}else {
			log.Info("html服务器开启成功", apiAddr, rtmpAddr)
		}
	}()
